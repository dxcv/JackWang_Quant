...
class frame_single:
    ...
    def process_closeposition(self, order):
        ...
        for i1 in range(0, ind):  # i1 + t_len 定位trade
            ...
            if self.trade.loc[t_len + i1, '开仓操作'] == '买开':
                self.trade.loc[t_len + i1, '平仓价'] = order['成交价']
            ...
            if t_len + i1 != 0:
                ...
                self.trade.loc[t_len + i1, '逐笔累计盈利'] = self.trade.loc[t_len + i1, '平仓盈亏']
    ...
    def backtest_record_save(self, file_name, strategy_name):
        filename = os.getcwd()
        files = os.path.join(filename, 'backtest_result', file_name)
